                    ...
                    <div class="mb-3 pb-3 border-bottom">
                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            <small class="text-secondary">
                                ...
                                Cập nhật: 
                                @php
                                    $updatedAt = $course->updated_at instanceof \Carbon\Carbon 
                                        ? $course->updated_at 
                                        : \Carbon\Carbon::parse($course->updated_at);
                                @endphp
                    ...
                    <div class="mb-3 pb-2 border-bottom">
                        <div class="d-flex justify-content-between align-items-start mb-1">
                            ...
                            <small class="text-danger">
                                ...
                                @php
                                    $deadline = $quiz->deadline instanceof \Carbon\Carbon 
                                        ? $quiz->deadline 
                                        : \Carbon\Carbon::parse($quiz->deadline);
                                @endphp
                                {{ $deadline->diffForHumans() }}
                            </small>
                        </div>
                        <small class="text-secondary d-block mb-2">
                            <i class="bi bi-book me-1"></i>{{ $quiz->lesson->course->title ?? '' }}
                        </small>
                        <div class="d-flex justify-content-between align-items-center">
                            <small class="text-secondary">
                                <i class="bi bi-question-circle"></i> {{ $quiz->questions_count }} câu
                            </small>
                            <a href="{{ route('student.quiz.show', $quiz) }}" class="btn btn-sm btn-warning">
                                Làm ngay
                            </a>
                        </div>
                    </div>
                ...
                @forelse($recentActivities ?? [] as $activity)
                    <div class="d-flex align-items-start mb-3 pb-2 border-bottom">
                        ...
                        <div class="flex-grow-1">
                            ...
                            <small class="text-secondary">
                                @php
                                    $createdAt = $activity->created_at instanceof \Carbon\Carbon 
                                        ? $activity->created_at 
                    ...
                    @forelse($achievements ?? [] as $achievement)
                        <div class="text-center" style="width: 70px;">
                            <div class="bg-light rounded-circle p-2 mb-1 mx-auto" style="width: 50px; height: 50px;">
                                <i class="bi bi-{{ $achievement->icon }} text-{{ $achievement->color }}" style="font-size: 24px;"></i>
                            </div>
                            ...
                            <tbody>
                                @foreach($recentResults as $result)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ route('student.quiz.result', $result->quiz) }}" class="btn btn-sm btn-outline-primary">
                                                Xem chi tiết
                                            </a>
                                        </td>
                                    </tr>
                ...
                <div class="row g-4">
                        ...
                        <div class="row g-2">
                            <div class="col-6">
                                <div class="bg-light p-2 rounded text-center">
                                    ...
                                    <span class="h5 mb-0">{{ $completedLessons ?? 0 }}</span>
                                </div>
                            </div>
                            <div class="col-6">
                                <div class="bg-light p-2 rounded text-center">
                                    <small class="text-secondary d-block">Quiz đã làm</small>
                                    <span class="h5 mb-0">{{ $completedQuizzes ?? 0 }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="bg-light p-3 rounded">
                            ...
                            <div class="mb-2">
                                <small class="text-secondary">Tổng số khóa học đã đăng ký</small>
                                <div class="fw-bold">{{ $enrolledCourses ?? 0 }}</div>
                            </div>
                            <div class="mb-2">
                                <small class="text-secondary">Khóa học đã hoàn thành</small>
                                <div class="fw-bold">{{ $completedCourses ?? 0 }}</div>
                            </div>
                            <div>
                                <small class="text-secondary">Quiz chưa làm</small>
                                <div class="fw-bold">{{ $pendingQuizzes ?? 0 }}</div>
                            </div>
                        </div>
                    </div>
                </div>
